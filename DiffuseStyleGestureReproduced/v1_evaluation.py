...
def evaluate_frechet_gesture_distance_sliding_diffusion(
    ...
    with torch.no_grad():
        ...
        val_loader.dataset.seq_length = model.num_of_pre_timestep_frames + 2 * model.num_of_timestep_frames + model.num_of_post_timestep_frames + evaluation_length
        ...
        result_tensor = torch.zeros(
            (num_samples, model.num_of_timestep_frames + evaluation_length, encoded_feature_dim),
            device=device,
        )
        ...
        for i in range(evaluation_length + model.num_of_timestep_frames):
...
def evaluate_frechet_gesture_distance_normal_diffusion(
    ...
    with torch.no_grad():
        ...
        for timestep in range(model.diffusion.number_of_timesteps-1, -1, -1):
            ...
            timestep_tensor = torch.tensor([timestep], dtype=torch.int64, device=device).expand(num_samples, 1)
