...
# Based on PYMO BVHWriter https://github.com/omimo/PyMO with some modifications
# to work with numpy instead of pandas dataframes
    ...
    def _printJoint(self, X, joint, tab, ofile):
        ...
        if n_channels > 0:
            for ci in range(len(pos)):
                cn = pos[ci]
                channel_name = '%s_%s' % (joint, cn)
                if channel_name in X.channel_names:
                    idx = X.channel_names.index(channel_name)
                    self.motions_.append(X.values[:, idx])
                    ch_str = ch_str + ' ' + cn 
