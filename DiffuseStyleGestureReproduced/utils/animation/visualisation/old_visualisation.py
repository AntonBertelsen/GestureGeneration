...
def display_viewer():
    html_file_path = f"http://localhost:{state['http_server_port']}/utils/animation/visualisation/bvh_viewer.html?wsport={state['websocket_port']}"
...
def add_pose_to_message(pose: torch.Tensor, skeleton: Skeleton, message: dict = None):
    # If no message is provided, create an empty dictionary
    if message is None:
        message = {}
    
    message["pose"] = skeleton.pose_to_websocket_format(pose)

    return message

def add_debug_positions_to_message(debug_positions: torch.Tensor, message: dict = {}):
...
def add_debug_text_to_message(debug_text: str, message: dict = {}):
    # Add the debug text to the message
    message["debug_text"] = debug_text
    return message

def add_debug_tensor_to_message(debug_tensor: torch.Tensor, debug_tensor_name: str = "", message: dict = {}):    
...
def add_audio_to_message(audio, sample_rate=44100, message: dict = {}):    
    try:
...
def add_character_to_message(character_name: str, position: tuple = (0, 0, 0), rotation: tuple = (0, 0, 0), color = (1, 1, 1), message: dict = {}):
    if "characters" not in message:
        message["characters"] = []
    ...
    message["characters"].append(character_data)
    return message